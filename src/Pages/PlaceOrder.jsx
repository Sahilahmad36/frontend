...
const Placeorder = () => {
  ...
  // Function to apply the 10% discount
  const applyDiscount = (amount) => {
    const discountPercentage = 0.10; // 10% discount
    return (amount * (1 - discountPercentage)).toFixed(2); // Return discounted amount
  };

  const initPay = (order) => {
    const options = {
      key: "rzp_test_NJCcgzETBoeRmJ",
      amount: order.amount,
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      let totalAmount = getCartAmount();  // Get the total amount
      const discountedAmount = applyDiscount(totalAmount);  // Apply the discount

      // Calculate the final amount with delivery fee for COD method
      const finalAmount = parseFloat(discountedAmount) + (method === 'cod' ? delivery_fee : 0);

      let orderData = {
        address: formData,
        items: orderItems,
        amount: finalAmount  // Use the final discounted amount here
      };
